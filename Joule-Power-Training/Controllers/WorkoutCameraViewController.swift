        ...
        if availableExercises.contains(action) && confidence >= 0.95 && exerciseDetected == false {
            if action == "Squat" {
                ...
                if repValidation.2 {
                    ...
                    let partialCompletedRep = PartialCompetedRep(timeArray: repValidation.0, velocityArray: repValidation.1)
                    partialCompletedReps.append(partialCompletedRep)
                    
//                    let completedRep = CompletedRep(timeArray: repValidation.0, velocityArray: repValidation.1, load: currentWorkout.targetLoad, targetVelocity: currentWorkout.targetVelocity)
//
                    print("Rep Count: \(repCount)")
                    print("Rep Time: \(repValidation.0)")
                    print("Smoothed Velocity: \(partialCompletedRep.velocityArray)")
                    
                    if repCount == currentWorkout.targetReps {
                        performSegue(withIdentifier: "trackerToSummary", sender: self)
                    }
                    
                    if partialCompletedRep.averageVelocity >= currentWorkout.targetVelocity {
                        pointsLayer.strokeColor = UIColor.green.cgColor
                        pointsLayer.fillColor = UIColor.green.cgColor
                    } else if partialCompletedRep.averageVelocity >= (currentWorkout.targetVelocity - 5){
                        pointsLayer.strokeColor = UIColor.yellow.cgColor
                        pointsLayer.fillColor = UIColor.yellow.cgColor
                    } else {
                        pointsLayer.strokeColor = UIColor.red.cgColor
                        pointsLayer.fillColor = UIColor.red.cgColor
